...
# Import OS shortcuts - handle missing PySide6 gracefully
try:
    from helpmesign.ui.components import get_os_shortcuts

    OS_SHORTCUTS_AVAILABLE = True
except (ImportError, OSError):
    # Mock the OS shortcuts function for CI environments
    def get_os_shortcuts():
        return {"cmd": "⌘", "option": "⌥", "shift": "⇧", "ctrl": "⌃"}

    OS_SHORTCUTS_AVAILABLE = False
...
def test_os_detection():
    ...
    if system == "Darwin":
        ...
        if "cmd" in shortcuts and "shift" in shortcuts:
            logger.info(
                f"• {shortcuts['cmd']}+{shortcuts['shift']}+K - Clear Text Input"
            )
