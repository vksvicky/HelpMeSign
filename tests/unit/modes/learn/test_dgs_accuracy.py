#!/usr/bin/env python3
"""
Tests for German Sign Language (DGS) gesture accuracy and viewing angles
Ensures DGS signs are accurate, viewable, and anatomically correct
"""
...
class TestDGSAccuracy:
    """Test German Sign Language gesture accuracy"""

    def test_dgs_alphabet_poses_are_anatomically_valid(self):
        """Test that DGS alphabet poses respect human anatomy"""
        validator = JointConstraintValidator()

        # DGS alphabet poses (simplified examples)
        dgs_alphabet = {
            "A": {
        ...
        for letter, pose in dgs_alphabet.items():
            ...
            for joint_name, hpr in constrained_pose.items():
                ...
                if constraint:
                    ...
                    ), f"DGS {letter} {joint_name} heading invalid"
                    assert (
                        constraint.min_p <= p <= constraint.max_p
                    ), f"DGS {letter} {joint_name} pitch invalid"
                    assert (
                        constraint.min_r <= r <= constraint.max_r
                    ), f"DGS {letter} {joint_name} roll invalid"

    def test_dgs_numbers_poses_are_valid(self):
        """Test that DGS number poses are anatomically valid"""
        validator = JointConstraintValidator()

        # DGS number poses (simplified examples)
        dgs_numbers = {
            "1": {
        ...
        for number, pose in dgs_numbers.items():
            constrained_pose = validator.validate_and_constrain_pose(pose)
            ...
            for joint_name, hpr in constrained_pose.items():
                ...
                if constraint:
                    ...
                    assert (
                        constraint.min_h <= h <= constraint.max_h
                    ), f"DGS {number} {joint_name} heading invalid"
                    assert (
                        constraint.min_p <= p <= constraint.max_p
                    ), f"DGS {number} {joint_name} pitch invalid"
                    assert (
                        constraint.min_r <= r <= constraint.max_r
                    ), f"DGS {number} {joint_name} roll invalid"

    def test_dgs_common_words_poses_are_valid(self):
        """Test that common DGS word poses are anatomically valid"""
        validator = JointConstraintValidator()

        # Common DGS word poses (simplified examples)
        dgs_words = {
            "HALLO": {
        ...
        for word, pose in dgs_words.items():
            constrained_pose = validator.validate_and_constrain_pose(pose)
            ...
            for joint_name, hpr in constrained_pose.items():
                ...
                if constraint:
                    ...
                    assert (
                        constraint.min_h <= h <= constraint.max_h
                    ), f"DGS {word} {joint_name} heading invalid"
                    assert (
                        constraint.min_p <= p <= constraint.max_p
                    ), f"DGS {word} {joint_name} pitch invalid"
                    assert (
                        constraint.min_r <= r <= constraint.max_r
                    ), f"DGS {word} {joint_name} roll invalid"


class TestDGSViewingAngles:
    """Test that DGS signs are viewable from appropriate angles"""

    def test_dgs_signs_visible_from_front_view(self):
        """Test that DGS signs are positioned for optimal front viewing"""
        validator = JointConstraintValidator()
    ...
    def test_dgs_signs_visible_from_side_view(self):
        """Test that DGS signs are positioned for optimal side viewing"""
        validator = JointConstraintValidator()
    ...
    def test_dgs_signs_visible_from_learner_perspective(self):
        """Test that DGS signs are positioned for optimal learning"""
        validator = JointConstraintValidator()
...
class TestDGSHandShapeAccuracy:
    """Test that DGS hand shapes are anatomically accurate"""

    def test_dgs_finger_extension_accuracy(self):
        """Test that DGS finger extensions are anatomically accurate"""
        validator = JointConstraintValidator()

        # Test different finger extension patterns used in DGS
        finger_patterns = {
    ...
    def test_dgs_thumb_position_accuracy(self):
        """Test that DGS thumb positions are anatomically accurate"""
        validator = JointConstraintValidator()

        # Test different thumb positions used in DGS
        thumb_positions = {
            "thumb_up": {
                "mixamorig:RightHandThumb1": [0, 0, 0],
...
class TestDGSBilateralSigning:
    """Test DGS bilateral signing accuracy and symmetry"""

    def test_dgs_bilateral_signs_maintain_symmetry(self):
        """Test that DGS bilateral signs maintain proper symmetry"""
        validator = JointConstraintValidator()
    ...
    def test_dgs_dominant_hand_preference_accuracy(self):
        """Test that DGS dominant hand preference is respected"""
        validator = JointConstraintValidator()
...
class TestDGSAnimationIntegration:
    """Integration tests for DGS animation accuracy"""

    def test_dgs_animation_manager_retrieves_pose_data(self):
        """Test that animation manager retrieves DGS pose data correctly"""
    ...
    def test_dgs_sign_sequence_maintains_accuracy(self):
        """Test that DGS sign sequences maintain anatomical accuracy"""
        validator = JointConstraintValidator()

        # Simulate a sequence of poses for signing a DGS word
        sign_sequence = [
            {
                "mixamorig:RightArm": [0, 0, 0],
                "mixamorig:RightHand": [0, 0, 0],
        ...
        for i, pose in enumerate(sign_sequence):
            ...
            for joint_name, hpr in constrained_pose.items():
                ...
                if constraint:
                    ...
                    assert (
                        constraint.min_h <= h <= constraint.max_h
                    ), f"DGS sequence step {i} {joint_name} heading invalid"
                    assert (
                        constraint.min_p <= p <= constraint.max_p
                    ), f"DGS sequence step {i} {joint_name} pitch invalid"
                    assert (
                        constraint.min_r <= r <= constraint.max_r
                    ), f"DGS sequence step {i} {joint_name} roll invalid"

    def test_dgs_sign_visibility_optimization(self):
        """Test that DGS signs are optimized for visibility"""
        validator = JointConstraintValidator()
