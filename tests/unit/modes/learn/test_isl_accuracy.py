#!/usr/bin/env python3
"""
Tests for Irish Sign Language (ISL) gesture accuracy and viewing angles
Ensures ISL signs are accurate, viewable, and anatomically correct
"""
...
class TestISLAccuracy:
    """Test Irish Sign Language gesture accuracy"""

    def test_isl_alphabet_poses_are_anatomically_valid(self):
        """Test that ISL alphabet poses respect human anatomy"""
        validator = JointConstraintValidator()

        # ISL alphabet poses (simplified examples)
        isl_alphabet = {
            "A": {
        ...
        for letter, pose in isl_alphabet.items():
            ...
            for joint_name, hpr in constrained_pose.items():
                ...
                if constraint:
                    ...
                    ), f"ISL {letter} {joint_name} heading invalid"
                    assert (
                        constraint.min_p <= p <= constraint.max_p
                    ), f"ISL {letter} {joint_name} pitch invalid"
                    assert (
                        constraint.min_r <= r <= constraint.max_r
                    ), f"ISL {letter} {joint_name} roll invalid"

    def test_isl_numbers_poses_are_valid(self):
        """Test that ISL number poses are anatomically valid"""
        validator = JointConstraintValidator()

        # ISL number poses (simplified examples)
        isl_numbers = {
            "1": {
        ...
        for number, pose in isl_numbers.items():
            constrained_pose = validator.validate_and_constrain_pose(pose)
            ...
            for joint_name, hpr in constrained_pose.items():
                ...
                if constraint:
                    ...
                    assert (
                        constraint.min_h <= h <= constraint.max_h
                    ), f"ISL {number} {joint_name} heading invalid"
                    assert (
                        constraint.min_p <= p <= constraint.max_p
                    ), f"ISL {number} {joint_name} pitch invalid"
                    assert (
                        constraint.min_r <= r <= constraint.max_r
                    ), f"ISL {number} {joint_name} roll invalid"

    def test_isl_common_words_poses_are_valid(self):
        """Test that common ISL word poses are anatomically valid"""
        validator = JointConstraintValidator()

        # Common ISL word poses (simplified examples)
        isl_words = {
            "HELLO": {
        ...
        for word, pose in isl_words.items():
            constrained_pose = validator.validate_and_constrain_pose(pose)
            ...
            for joint_name, hpr in constrained_pose.items():
                ...
                if constraint:
                    ...
                    assert (
                        constraint.min_h <= h <= constraint.max_h
                    ), f"ISL {word} {joint_name} heading invalid"
                    assert (
                        constraint.min_p <= p <= constraint.max_p
                    ), f"ISL {word} {joint_name} pitch invalid"
                    assert (
                        constraint.min_r <= r <= constraint.max_r
                    ), f"ISL {word} {joint_name} roll invalid"


class TestISLViewingAngles:
    """Test that ISL signs are viewable from appropriate angles"""

    def test_isl_signs_visible_from_front_view(self):
        """Test that ISL signs are positioned for optimal front viewing"""
        validator = JointConstraintValidator()
    ...
    def test_isl_signs_visible_from_side_view(self):
        """Test that ISL signs are positioned for optimal side viewing"""
        validator = JointConstraintValidator()
    ...
    def test_isl_signs_visible_from_learner_perspective(self):
        """Test that ISL signs are positioned for optimal learning"""
        validator = JointConstraintValidator()
...
class TestISLHandShapeAccuracy:
    """Test that ISL hand shapes are anatomically accurate"""

    def test_isl_finger_extension_accuracy(self):
        """Test that ISL finger extensions are anatomically accurate"""
        validator = JointConstraintValidator()

        # Test different finger extension patterns used in ISL
        finger_patterns = {
    ...
    def test_isl_thumb_position_accuracy(self):
        """Test that ISL thumb positions are anatomically accurate"""
        validator = JointConstraintValidator()

        # Test different thumb positions used in ISL
        thumb_positions = {
            "thumb_up": {
                "mixamorig:RightHandThumb1": [0, 0, 0],
...
class TestISLBilateralSigning:
    """Test ISL bilateral signing accuracy and symmetry"""

    def test_isl_bilateral_signs_maintain_symmetry(self):
        """Test that ISL bilateral signs maintain proper symmetry"""
        validator = JointConstraintValidator()
    ...
    def test_isl_dominant_hand_preference_accuracy(self):
        """Test that ISL dominant hand preference is respected"""
        validator = JointConstraintValidator()
...
class TestISLAnimationIntegration:
    """Integration tests for ISL animation accuracy"""

    def test_isl_animation_manager_retrieves_pose_data(self):
        """Test that animation manager retrieves ISL pose data correctly"""
        ...
        # Mock ISL sign data with valid values
    ...
    def test_isl_sign_sequence_maintains_accuracy(self):
        """Test that ISL sign sequences maintain anatomical accuracy"""
        validator = JointConstraintValidator()

        # Simulate a sequence of poses for signing an ISL word
        sign_sequence = [
            {
                "mixamorig:RightArm": [0, 0, 0],
                "mixamorig:RightHand": [0, 0, 0],
        ...
        for i, pose in enumerate(sign_sequence):
            ...
            for joint_name, hpr in constrained_pose.items():
                ...
                if constraint:
                    ...
                    assert (
                        constraint.min_h <= h <= constraint.max_h
                    ), f"ISL sequence step {i} {joint_name} heading invalid"
                    assert (
                        constraint.min_p <= p <= constraint.max_p
                    ), f"ISL sequence step {i} {joint_name} pitch invalid"
                    assert (
                        constraint.min_r <= r <= constraint.max_r
                    ), f"ISL sequence step {i} {joint_name} roll invalid"

    def test_isl_sign_visibility_optimization(self):
        """Test that ISL signs are optimized for visibility"""
        validator = JointConstraintValidator()
